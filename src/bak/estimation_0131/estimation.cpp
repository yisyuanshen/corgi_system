...
int main(int argc, char* argv[]) {
    ...
    ContactMap cm;
    for (int i = 1; i < n - 1; i++) {
        bool update = counter % 40 == 0? true: false;
        // std::cout << i << "\n";
        ENCODER_DATA elf = {
            df.iloc("lf.beta", i),
            df.iloc("lf.theta", i),
        ...
        double alpha_l = atan2((df.iloc("lh.dist", i) + 5e-3 * random_number() - df.iloc("lf.dist", i) - 5e-3 * random_number()) , 0.4);
        double alpha_r = atan2((df.iloc("rh.dist", i) + 5e-3 * random_number() - df.iloc("rf.dist", i) - 5e-3 * random_number()) , 0.4);
        DST_DATA dlf = {
            df.iloc("lf.dist", i) + 5e-3 * random_number(),
            alpha_l
        };
        DST_DATA drf = {
            df.iloc("rf.dist", i) + 5e-3 * random_number(),
            alpha_r
        };
        DST_DATA drh = {
            df.iloc("rh.dist", i) + 5e-3 * random_number(),
            alpha_r
        };
        DST_DATA dlh = {
            df.iloc("lh.dist", i) + 5e-3 * random_number(),
            alpha_l
        };
        IMU_DATA imu = {
            Eigen::Vector3d(df.iloc("a.x", i), df.iloc("a.y", i), df.iloc("a.z", i)) + 3e-3 * random_vector<3>() + Eigen::Vector3d(1e-3, 1e-3, 1e-3),
            Eigen::Vector3d(df.iloc("w.x", i), df.iloc("w.y", i), df.iloc("w.z", i)) + 2e-3 * random_vector<3>() + Eigen::Vector3d(1e-3, 1e-3, 1e-3),
        ...
        rf_leg.leg.Calculate(df.iloc("rf.theta", i), df.iloc("rf.theta_d", i), 0, df.iloc("rf.beta", i), df.iloc("rf.beta_d", i), 0);
        rf_leg.leg.PointContact(cm.lookup(df.iloc("rf.theta", i), df.iloc("rf.beta", i) + alpha_r), alpha_r);
        rf_leg.leg.PointVelocity(rot.transpose() * Eigen::Vector3d(u[0], u[1], u[2]), Eigen::Vector3d(df.iloc("w.x", i), df.iloc("w.y", i), df.iloc("w.z", i)),
            cm.lookup(df.iloc("rf.theta", i), df.iloc("rf.beta", i) + alpha_r), alpha_r,
            false
        );

        rh_leg.leg.Calculate(df.iloc("rh.theta", i), df.iloc("rh.theta_d", i), 0, df.iloc("rh.beta", i), df.iloc("rh.beta_d", i), 0);
        rh_leg.leg.PointContact(cm.lookup(df.iloc("rh.theta", i), df.iloc("rh.beta", i) + alpha_r), alpha_r);
        rh_leg.leg.PointVelocity(rot.transpose() * Eigen::Vector3d(u[0], u[1], u[2]), Eigen::Vector3d(df.iloc("w.x", i), df.iloc("w.y", i), df.iloc("w.z", i)),
            cm.lookup(df.iloc("rh.theta", i), df.iloc("rh.beta", i) + alpha_r), alpha_r,
            false
        );

        lh_leg.leg.Calculate(df.iloc("lh.theta", i), df.iloc("lh.theta_d", i), 0, df.iloc("lh.beta", i), df.iloc("lh.beta_d", i), 0);
        lh_leg.leg.PointContact(cm.lookup(df.iloc("lh.theta", i), df.iloc("lh.beta", i) + alpha_l), alpha_l);
        lh_leg.leg.PointVelocity(rot.transpose() * Eigen::Vector3d(u[0], u[1], u[2]), Eigen::Vector3d(df.iloc("w.x", i), df.iloc("w.y", i), df.iloc("w.z", i)),
            cm.lookup(df.iloc("lh.theta", i), df.iloc("lh.beta", i) + alpha_l), alpha_l,
            false
        );
        ...
        estimate_state.row(i).segment(12, 3) = lf_leg.leg.contact_velocity;
        estimate_state.row(i).segment(15, 3) = rf_leg.leg.contact_velocity;
        estimate_state.row(i).segment(18, 3) = rh_leg.leg.contact_velocity;
        estimate_state.row(i).segment(21, 3) = lh_leg.leg.contact_velocity;
